---
description: Goal arbiter / judge. Called by the Planner when the Planner suspects the project's main goal (PROJECT.yaml.goal) might be done. Reads the goal text, the full backlog (pending + done), the tasks history, and a sample of the codebase, then returns a structured verdict — either "continue" (with concrete reason what's still missing) or "complete" (with concrete reason what was actually delivered against the goal). Read-only — cannot edit code, cannot write to db, cannot call any other agent. Decoupled from Planner so Planner can't shortcut to "done" out of laziness.
mode: subagent
